...
class DenseStack_Backnone(nn.Module):
    def __init__(self, input_channel=128, out_channel=24, latent_size=256, kpts_num=21, pretrain=True):
        ...
        self.mobilebetv2 = mobile_netv2.features
        mobilenet_v3_small = models.mobilenet_v3_small(pretrained=True)
        self.mobilenetv3 = mobilenet_v3_small.features
        efficientnet = models.efficientnet_b0(pretrained=True)
        self.efficientnetb0 = nn.Sequential(*list(efficientnet.features)[:])
    ...
    def forward(self, x):
        ...
        # post quantize the flow 
        x = self.quant(x)
        mobileNetV3_output = self.mobilenetv3(x)
        ...
        uv_reg = self.fc(uv_reg)
        return self.dequant(latent), self.dequant(uv_reg)


        # # this is efficientNet b2
        # efficientnet_output = self.efficientnetb0(x)    #first layer = 1,1280,4,4
        # latent = self.conv_branch5[0](efficientnet_output)
        # latent = self.conv_branch5[1](latent)
        # uv_reg = self.conv_branch6(efficientnet_output)
        # uv_reg = uv_reg.view(uv_reg.shape[0], uv_reg.shape[1], -1)
        # uv_reg = self.fc(uv_reg)     # the output shape is 21x16 using fc to flatten to 21,2
        # return latent, uv_reg
